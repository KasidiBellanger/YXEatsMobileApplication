  ...
  [[session dataTaskWithRequest:searchRequest completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
    ...
    if (!error && httpResponse.statusCode == 200) {
      ...
      if ([businessArray count] > 0) {
        ...
        NSMutableDictionary* myFirstDict = [[NSMutableDictionary alloc]init];
          NSUInteger indexKey = 0U;
          for (objectInstance in businessArray){
              [myFirstDict setObject:objectInstance forKey:[NSNumber numberWithUnsignedLong:indexKey++]];
          }
        ...
        NSLog(@"%lu businesses found, querying business info for the top result: %@", (unsigned long)[businessArray count], firstBusinessID);
          for(long i=0; i<indexKey; i++){
              [self queryBusinessInfoForBusinessId:[NSString stringWithFormat:@"%lu", i] completionHandler:completionHandler];
          }
      } else {
